...
def authenticate_user(db: Session, username: str, password: str) -> Optional[User]:
    ...
    user = (
        db.query(User)
        .filter(func.lower(User.username) == func.lower(func.cast(username, User.username.type)))
        .first()
    )
...
def verify_access_token(token: str, db: Session) -> Optional[User]:
    ...
    # Local import to avoid any potential circular import during app startup
